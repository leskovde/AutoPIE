...
Giri\change[]{TODO: Read the notes and the overview, summarize the contents.}
DG\change[]{TODO: Read the notes and the overview, summarize the contents.}
...
For example, the \icode{VariantGenerationConsumer} \change[]{TODO: Change to VariantGeneratingConsumer} 
...
The \icode{VariantGenerationConsumer} unifies the interface between the two 
...
They might perform more complicated \icode{Rewriter} actions based on the 
current node type.
...
the \icode{DependencyMappingConsumer} collects its output, and the variant 
...
The \icode{DependencyMappingConsumer} has determined $n$: the number of code 
units in the file.
