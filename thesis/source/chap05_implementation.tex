...
The following text references source files of the implementation. 
The implementation - named AutoPIE (Automated Problem-Investigating 
Execution) - is a part of Attachments~\ref{chap:attachments}. 
More details on those files and their structure can be found in 
Attachment~\ref{chap:tree}.
...
CMake.
The Python API, which requires Python 3, and its C++ scripting bridge can 
also be included by specifying a few other arguments.
...
It is required to run slicer implementations and is invoked from Python.
This usage indicates the fact that the user must also have Docker's Python API installed.

Technologies and their system requirements are described in more detail 
in Attachment~\ref{chap:userdoc}.
...
This design choice of implementing multiple components is directly reflected 
by the project's tree structure seen in Attachment~\ref{chap:tree}.

\begin{figure}[h]\centering
\includegraphics[scale=0.85]{components}
\caption{Communication between modules in three different pipelines:
the naive approach (orange), the Delta-based approach (blue), and 
the slicing-based approach (red and green).}
...
\begin{itemize}
  ...
  The class \icode{DependencyGraph} supports visualization of the current 
  input. 
  The user can view how the source file is structured. 
  Moreover, the visualization also showcases the dependencies of individual 
  code snippets - both in terms of nesting and simple data flow.
...
\paragraph{DG.} We chose DG~\citep{Chalupa20} as it is the most reliable 
...
injection.
Moreover, the slicer also does not handle failing programs. 
The tool does not generate slices for programs resulting in runtime errors. 
We have made a workaround for this issue in the form statement injection. 
We place an \icode{exit} call just before the error-inducing statement, 
terminating before any errors are thrown.

Giri's GitHub repository can be found at
