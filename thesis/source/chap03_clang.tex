...
In the example shown in~\ref{lst:astconsumer}, the consumer passes variable 
references to the visitor. 
...
On the one hand, it is possible to reach the \icode{BuiltinType} by 
calling \icode{getPointeeLoc()}. 
The result is a \icode{BuiltinTypeLoc}, through which a \icode{QualType} 
can be extracted. 
The qualifier leads to the desired \icode{BuiltinType}. 
On the other hand, one can extract a \icode{QualType} from 
the starting \icode{PointerTypeLoc} and use it to get 
a \icode{PointerType} instance. 
By calling \icode{getPointeeType()}, it is possible to get to 
the \icode{QualType} that leads to the desired built-in type. 

Both of these approaches start at the same source location and end 
with the same built-in type. 
...
\change[inline]{TODO: Show an example of instrumentation code.}
