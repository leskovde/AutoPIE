...
namespace Common
{
	...
	{
		if (prompt && std::filesystem::exists(Common::TempFolder))
		{
			out::All() << "WARNING: The path " << Common::TempFolder << " exists and is about to be cleared! Do you want to proceed? [Y/n] ";
		...
		out::All() << "Clearing the " << Common::TempFolder << " directory...\n";

		std::filesystem::remove_all(Common::TempFolder);
		std::filesystem::create_directory(Common::TempFolder);
	...
	int Compile(const std::filesystem::directory_entry& entry, const clang::Language language)
	{
		...
		const auto output = Common::TempFolder + entry.path().filename().replace_extension(".out").string();
	...
	bool ValidateResults(GlobalContext& globalContext)
	{
		...
		for (const auto& entry : std::filesystem::directory_iterator(Common::TempFolder))
		{
		...
		for (const auto& entry : files)
		{
			...
			launchInfo.SetWorkingDirectory(Common::TempFolder);
			launchInfo.SetLaunchFlags(lldb::eLaunchFlagExec | lldb::eLaunchFlagDebug);

			const auto executable = Common::TempFolder + entry.path().filename().replace_extension(".out").string();
			...
			out::Verb() << "process.GetState()           = " << Common::StateToString(process.GetState()) << "\n";
			...
			while (!done)
			{
				...
				if (listener.WaitForEvent(timeOut /*seconds*/, event))
				{
					if (lldb::SBProcess::EventIsProcessEvent(event))
					{
						...
						{
							out::Verb() << "Invalid process event: " << Common::StateToString(state) << "\n";
						}
						else
						{
							out::Verb() << "Process state event changed to: " << Common::StateToString(state) << "\n";
							...
							if (state == lldb::eStateStopped)
							{
								...
								out::Verb() << "thread.GetName()             = " << (thread.GetName() != nullptr
									? thread.GetName()
									: "(null)") << "\n";
								out::Verb() << "thread.GetStopReason()       = " << Common::StopReasonToString(thread.GetStopReason()) <<
									"\n";
		...
		const auto newFileName = Common::TempFolder + std::string("autoPieOut") + Common::LanguageToExtension(globalContext.language);
