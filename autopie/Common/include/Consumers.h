...
namespace Common
{
	...
	public:
		explicit VariantPrintingASTConsumer(clang::CompilerInstance* ci, const int errorLine) : visitor_(std::make_unique<VariantPrintingASTVisitor>(ci, errorLine))
		{
		}
		 ...
		 */
		void SetData(SkippedMapRef skippedNodes, DependencyGraph graph, std::vector<int> errorLines) const
		{
		 ...
		 */
		std::vector<int> GetAdjustedErrorLines() const
		{
	...
	public:
		...
		[[nodiscard]] std::vector<int> GetPotentialErrorLines() const
		{
